/**
 * Deterministic double for Pre-Visit read-aloud.
 *
...
export const FAKE_PREVISIT_SPEECH_BEHAVIOURS = Object.freeze({
  AUDIO: "audio",
  EMPTY: "empty",
  NOT_AUDIO: "not_audio",
...
export function fakePreVisitSpeechAudio() {
...
export function createFakePreVisitSpeechProvider(options = {}) {
  const behaviour = options.behaviour || FAKE_PREVISIT_SPEECH_BEHAVIOURS.AUDIO;
  const calls = [];
  ...
  return {
    ...
    async synthesize(request) {
      ...
      switch (behaviour) {
        case FAKE_PREVISIT_SPEECH_BEHAVIOURS.SERVER_ERROR:
          throw new PreVisitSpeechError(PREVISIT_SPEECH_ERRORS.PROVIDER_FAILED, { status: 503 });
        case FAKE_PREVISIT_SPEECH_BEHAVIOURS.TIMEOUT:
          throw new PreVisitSpeechError(PREVISIT_SPEECH_ERRORS.PROVIDER_FAILED, { reason: "timeout" });
        case FAKE_PREVISIT_SPEECH_BEHAVIOURS.EMPTY:
          return { audio: Buffer.alloc(0) };
        case FAKE_PREVISIT_SPEECH_BEHAVIOURS.NOT_AUDIO:
          return { audio: Buffer.from("<html>rate limited</html>".repeat(20), "utf8") };
        case FAKE_PREVISIT_SPEECH_BEHAVIOURS.AUDIO:
        default:
          return { audio: fakePreVisitSpeechAudio() };
      }
    },
  };
}

...
export async function deletePracticeWithArchivedContext(input) {
  ...
  const { checkPracticeDeletionBlockers, CONTEXTUAL_DATA_BLOCKED, blockingErrorCode } =
  ...
  const blockerCode = blockingErrorCode(blockers);
  if (blockerCode) {
    // Includes the guard's newer categories (issued medication plans and
    // prescriptions, unreleased document shares): archiving does not cover
    // those, and they are meant to stop the deletion.
    const err = new Error(CONTEXTUAL_DATA_BLOCKED);
    err.blockerReport = blockers;
    err.clientCode = blockerCode;
    throw err;
  }

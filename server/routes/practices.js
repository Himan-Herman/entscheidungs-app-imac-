...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (err?.message === CONTEXTUAL_DATA_BLOCKED) {
      ...
      // Specific code when the guard names one (main's newer blockers, e.g. a
      // live document release), the stable generic one otherwise.
      return res.status(409).json({ ok: false, error: err.clientCode || "practice_deletion_blocked" });
    }

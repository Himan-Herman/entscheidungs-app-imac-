...
const STANDARD_TEMPLATE_STRUCTURE = {
  ...
  sections: [
    // ── 1. Aktuelles Anliegen ─────────────────────────────────────────────────
    {
      ...
      questions: [
        {
          type: "textarea", isRequired: true, responseMaxLength: 1000,
        ...
        {
          type: "text", isRequired: true, responseMaxLength: 250,
        ...
        {
          type: "text", isRequired: false, responseMaxLength: 250,
        ...
        {
          type: "textarea", isRequired: false, responseMaxLength: 500,
    ...
    // ── 2. Beschwerden und Verlauf ───────────────────────────────────────────
    {
      ...
      questions: [
        ...
        {
          type: "textarea", isRequired: false, responseMaxLength: 800,
    ...
    // ── 3. Medikamente und Allergien ─────────────────────────────────────────
    {
      ...
      questions: [
        {
          type: "textarea", isRequired: false, responseMaxLength: 1000,
...
async function buildSectionsInTx(tx, templateId, sections) {
  for (let sIdx = 0; sIdx < sections.length; sIdx++) {
    ...
    for (let qIdx = 0; qIdx < qs.length; qIdx++) {
      ...
      const rml = typeof q.responseMaxLength === "number"
        ? Math.min(3000, Math.max(50, Math.round(q.responseMaxLength)))
        : null;
      await tx.practiceAnamnesisQuestion.create({
        data: {
          ...
          responseMaxLength: (q.type === "text" || q.type === "textarea") ? rml : null,
          sortOrder: qIdx,
        },
      });
    }

...
test("a migrated sub-page IS carried across (Phase 2E.1 / 2E.2 / 2E.3)", () => {
  ...
  assert.equal(
    switchTargetPath("/patient/practice/link-a/inbox", "link-b"),
    "/patient/practice/link-b/inbox",
  );
  assert.equal(
...
test("a sub-page that does NOT exist in every context falls back to the home", () => {
  ...
  assert.equal(
    switchTargetPath("/patient/practice/link-a/inbox/item-1", "link-b"),
    "/patient/practice/link-b",
  );
  assert.equal(
    switchTargetPath("/patient/practice/link-a/telemedicine/session-1", "link-b"),
    "/patient/practice/link-b",
  );
});

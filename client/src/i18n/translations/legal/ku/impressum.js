/** Wergerandina agahdarî; referansên qanûnî yên Alman dema tê gotin. */
export default {
  pageTitle: "MedScoutX — Agahî ya qanûnî (Impressum)",
  title: "Agahî ya qanûnî (Impressum)",
  subtitle: "Agahî li gorî § 5 DDG û § 18 beşa 2 MStV (Almanya)",
  s1Heading: "1. Bersipar",
  addressStrong: "Himan Khorshidi",
  addressLines: ["Eisenstraße 64", "40227 Düsseldorf", "Almanya"],
  ddLegalForm: "Karsazê takekesî",
  emailDisplay: "himankhorshidy@gmail.com",
  emailHref: "mailto:contact@medscoutx.com",
  phoneDisplay: "+49 211 15895272",
  phoneHref: "tel:+4921115895272",
  privacyHref: "/datenschutz",
  backRegister: "Vegere tomarkirinê",
  backRegisterAria: "Vegere rûpela tomarkirinê",
  labelEmail: "E-name",
  labelPhone: "Telefon",
  labelLegalForm: "Forma qanûnî",
  labelResponsible: "Bersiparê naverokê li gorî § 18 beşa 2 MStV",
  ddResponsibleValue: "Himan Khorshidi, navnîşana li jor",
  s1Footnote:
    "Ev agahî ya qanûnî ji sepanê MedScoutX û malperên girêdayî derbas dibe ku sepan her wiha tê pêşkêş an ragihandin (mînak rûpelên Apple App Store û Google Play Store).",
  s2Heading: "2. Bersiva naverokê",
  s2p1:
    "Wek pêşkêşkerê xizmetê, ez ji naveroka xwe ya li ser van rûpelan li gorî qanûna giştî (§ 7 beşa 1 DDG) bersipar im. Lê li gorî §§ 8 heta 10 DDG ez ne mecbûr im ku agahiyên partiya sêyemîn ên hatine şandin an hatine hilanîn kontrol bikim an lê bigerim ka çalakiya neteqanûnî heye.",
  s2p2:
    "Pêdiviyên qanûnî yên rakirin an astengkirina bikaranîna agahiyan dewam dikin. Bersiva tenê dema naskirina binpêkirinê dest pê dike. Piştî ragihandinê ev naverok bêyî derengî nebinî werin rakirin.",
  s3Heading: "3. Bersiva girêdanan",
  s3p1:
    "Sepana MedScoutX dikare girêdanên malperên derve yên partiya sêyemîn hebin ku ez bandorê li naveroka wan nakim; ji ber vê yekê ez garantî ji bo naveroka derve nadin. Her dem operatorê rûpelên girêdayî ji naveroka wan bersipar e.",
  s3p2:
    "Kontrola domdar a rûpelên girêdayî bê delîlên diyarkirî yên binpêkirinê ne mantiqî ye. Dema binpêkirinên hatine zanîn ev girêdan derhal werin rakirin.",
  s4Heading: "4. Çareserkirina nakokiyan",
  s4p1Before:
    "Komîsyona Ewropî platformek ji bo çareserkirina nakokiyan ser înternetê pêşkêş dike: ",
  s4p1After: ".",
  s4p2:
    "Ez ne mecbûr im û ne jî dilxwaz im ku di prosedura çareserkirina nakokiyan li berendekek konsumatûr beşdar bibim.",
  s5Heading: "5. Mafên çapkirinê",
  s5p1:
    "Naverok û karên ku ji aliyê operatorê ve di vê sepana de hatine çêkirin bin qanûna çapkirinê ya Almanî ne. Ji bilî sînorên qanûna çapkirinê vekolîn, sererastkirin, belavkirin an bikaranîna din ji bo destûra nivîskî ya xwedî maf pêşî hewce ye.",
  s5p2:
    "Heke naveroka sepana ji aliyê operatorê ve nehatibe çêkirin, mafên partiya sêyemîn tên parastin. Heke binpêkirinê bibînî, ji kerema xwe min agahdar bike. Piştî zanînê ev naverok bêyî derengî werin rakirin.",
  s5p3:
    "Kredîtên wêne û îkonan: React Icons, Google Fonts (Roboto Slab, herêmî), grafîkên taybet û hêman logoya MedScoutX.",
  s6Heading: "6. Not li ser parastina daneyan",
  s6p1Before:
    "Li gorî § 38 BDSG ji bo MedScoutX tune ye ku baldarê taybet yên parastina daneyan were erêkirin. Daxwazên li ser nepênîtiyê dikarin rasterast ji bersiparê li jor werin şandin. Kûrahiya çawa daneyên kesane tên pêvajo kirin di ",
  s6privacyLink: "polîtîkaya nepênîtiyê",
  s6p1After: " de ye.",
};

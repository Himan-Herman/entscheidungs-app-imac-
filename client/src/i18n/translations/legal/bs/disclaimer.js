export default {
  pageTitle: "MedScoutX — Odricanje odgovornosti",
  title: "Odricanje odgovornosti / Napomena",
  subtitle: "Posljednje ažuriranje: 29. novembar 2025.",
  backRegister: "Nazad na registraciju",
  backAria: "Nazad na stranicu registracije",
  sections: [
    {
      id: "disc-1-zweck",
      heading: "1. Svrha aplikacije",
      blocks: [
        {
          type: "p",
          text:
            "MedScoutX je informacioni i orijentacioni alat uz podršku vještačke inteligencije u oblasti zdravstva. Aplikacija je namijenjena isključivo da korisnicima pruži početni strukturisani pregled mogućih uzroka tegoba i potencijalno relevantnih medicinskih specijalnosti.",
        },
        {
          type: "p",
          text:
            "MedScoutX nije medicinski uređaj u smislu EU Uredbe o medicinskim uređajima (MDR) i nije namijenjena postavljanju dijagnoza, preporuci liječenja ili zamjeni kliničkih odluka.",
        },
      ],
    },
    {
      id: "disc-2-keinediagnose",
      heading: "2. Bez medicinske dijagnoze ili liječenja",
      blocks: [
        {
          type: "p",
          text:
            "Izlazi i napomene koje pruža MedScoutX zasnivaju se na algoritmskoj obradi i statističkim modelima. Ne predstavljaju dijagnozu i nikada ne zamjenjuju lični pregled, vođenje ili liječenje od strane kvalifikovanih zdravstvenih radnika.",
        },
        {
          type: "p",
          text:
            "Odluke o dijagnostici, liječenju ili lijekovima ne smiju se donositi isključivo na osnovu izlaza vještačke inteligencije.",
        },
      ],
    },
    {
      id: "disc-3-notfall",
      heading: "3. Nije za hitne slučajeve",
      blocks: [
        {
          type: "p",
          text:
            "MedScoutX nije prikladan za hitne situacije. Ako imate akutne simptome koji ugrožavaju život, odmah kontaktirajte hitnu medicinsku pomoć ili birajte lokalni broj za hitne slučajeve (npr. 112 u EU ili 911 u SAD).",
        },
      ],
    },
    {
      id: "disc-4-grenzen",
      heading: "4. Granice obrade vještačke inteligencije",
      blocks: [
        {
          type: "p",
          text:
            "AI modeli mogu sadržavati greške, davati nepotpune informacije ili pogrešno tumačiti simptome. Korisnost izlaza zavisi od kvaliteta onoga što unesete (npr. opisi, odabrana regija tijela, kvalitet slike).",
        },
        {
          type: "p",
          text:
            "Predane informacije obrađuju se automatski. Ne postoji sveobuhvatna medicinska procjena vaše ukupne situacije.",
        },
      ],
    },
    {
      id: "disc-5-keinverhaeltnis",
      heading: "5. Bez odnosa lijekar–pacijent",
      blocks: [
        {
          type: "p",
          text:
            "Korištenje MedScoutX ne uspostavlja odnos lijekar–pacijent. Izlazi AI ne predstavljaju medicinski savjet.",
        },
      ],
    },
    {
      id: "disc-6-haftung",
      heading: "6. Ograničenje odgovornosti",
      blocks: [
        {
          type: "p",
          text:
            "Pružalac odgovara za štetu nastalu korištenjem ili nemogućnošću korištenja aplikacije samo u slučajevima namjere ili grube nepažnje. Odgovornost za indirektnu štetu, posljedičnu štetu ili izgubljenu dobit isključena je osim ako obavezno pravo drugačije ne određuje.",
        },
      ],
    },
    {
      id: "disc-7-alter",
      heading: "7. Međunarodna upotreba i dobni zahtjevi",
      blocks: [
        {
          type: "p",
          text:
            "MedScoutX je namijenjen isključivo korisnicima starijim od 16 godina. U pojedinim zemljama mogu važiti stroža pravila; npr. korisnici u SAD trebaju koristiti aplikaciju tek od 18 godina i uz saglasnost roditelja ili staratelja.",
        },
        {
          type: "p",
          text:
            "Korisnici mlađi od 18 godina trebaju koristiti MedScoutX samo uz saglasnost roditelja ili staratelja.",
        },
      ],
    },
    {
      id: "disc-8-automatisch",
      heading: "8. Bez automatizovanog donošenja odluka",
      blocks: [
        {
          type: "p",
          text:
            "MedScoutX ne donosi odluke u smislu čl. 22 GDPR-a koje proizvode pravne ili slično značajne efekte. Svi izlazi AI služe isključivo orijentaciji.",
        },
      ],
    },
    {
      id: "disc-9-aenderungen",
      heading: "9. Izmjene ovog odricanja",
      blocks: [
        {
          type: "p",
          text:
            "Pružalac može ažurirati ovo odricanje gdje to zahtijeva zakon, tehnologija ili izmjene funkcionalnosti aplikacije. Aktuelna verzija je uvijek dostupna u aplikaciji.",
        },
      ],
    },
    {
      id: "disc-10-kontakt",
      heading: "10. Kontakt za pitanja",
      blocks: [
        {
          type: "richContact",
          intro:
            "Za tehničke probleme, pitanja o korištenju ili nesigurnosti u vezi izlaza AI možete kontaktirati podršku:",
          emailLabel: "E-pošta:",
          phoneLabel: "Telefon:",
          noteBefore: "Napominjemo da kroz ovaj kontakt kanal ne može biti pružen ",
          noteStrong: "medicinski savjet",
          noteAfter: ".",
        },
      ],
    },
  ],
};

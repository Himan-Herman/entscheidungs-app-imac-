...
Legal package: `COMPLETE` 2026-09-18 — internal legal pre-assessment with sourced analysis, sign-off sheet restructured to A–L · B4 `PENDING EXTERNAL LEGAL DETERMINATION` · B5 `DPIA DRAFT COMPLETE / FINAL APPROVAL PENDING`
...
- `PENDING EXTERNAL LEGAL DETERMINATION` and `DPIA DRAFT COMPLETE / FINAL
  APPROVAL PENDING` — sub-states of `OPEN`: everything that can be prepared
  internally is prepared, and the external decision is the only thing missing.
  Neither is nearer to `VERIFIED` than `OPEN`.
...
| B4 | Legal basis determined and documented | **`PENDING EXTERNAL LEGAL DETERMINATION`** | Legal review packet prepared on 2026-08-17 — [`DOCUMENT_TRANSLATION_LEGAL_REVIEW_PACKET.md`](DOCUMENT_TRANSLATION_LEGAL_REVIEW_PACKET.md). Internal legal pre-assessment 2026-09-18 — [`DOCUMENT_TRANSLATION_LEGAL_ASSESSMENT.md`](DOCUMENT_TRANSLATION_LEGAL_ASSESSMENT.md): roles per processing stage with three variants, Art. 6 and Art. 9 options, consent variants A/B, a recommended question for counsel, each point tagged FACT / INTERPRETATION / OPEN / EXTERNAL against primary sources. | 2026-09-18 | no | — | Not to be asserted from code. A pre-assessment is preparation, **not** evidence — the row counts as `OPEN` until the sign-off sheet (fields 1–5) is signed. |
| B5 | DPIA/DSFA necessity assessed for this processing | **`DPIA DRAFT COMPLETE / FINAL APPROVAL PENDING`** | Full DPIA draft prepared 2026-09-18 — [`DOCUMENT_TRANSLATION_DPIA_DRAFT.md`](DOCUMENT_TRANSLATION_DPIA_DRAFT.md). Processing, purposes, data categories, data subjects, recipients, steps, necessity, 17 named risks, 20 technical and 9 organisational safeguards. Necessity pre-assessed internally against WP248 rev.01 and the DSK list: criteria 4, 7, 8 met, 5 open → `LIKELY REQUIRED`. | 2026-09-18 | partial | operator | The repository contained no DPIA and no template before this. **Residual-risk assessment and the sign-off field are deliberately left empty**; the necessity finding itself belongs to the controller — a complete draft is preparation, not evidence. |
...
| B7 | Patient-facing information decided (what is shown before the first run) | `OPEN` | German draft prepared 2026-09-18 — [`DOCUMENT_TRANSLATION_LEGAL_DECISION_MATRIX.md`](DOCUMENT_TRANSLATION_LEGAL_DECISION_MATRIX.md) Annex A; completed the same day with possible errors, the PDF export and the pointer to the privacy notice. Legal-basis line deliberately left as a marked variant. | 2026-09-18 | partial | operator | Depends on B3/B4. The UI has **no slot** for pre-run information today — that is a contained follow-up change (one element, one i18n key × 6 languages) once the text is approved. Translations only after the German text is signed off. |
...
|---|---|
| [`DOCUMENT_TRANSLATION_LEGAL_SIGNOFF.md`](DOCUMENT_TRANSLATION_LEGAL_SIGNOFF.md) | **The one document a reviewer needs.** Sections A–L: facts, data flow, safeguards, 13 decision fields plus two supplementary questions (MDR, AI Act) — each with a non-binding internal pre-assessment, options and a comment field — conditions, a four-option overall decision and the signature block. Everything else is an appendix |
| [`DOCUMENT_TRANSLATION_LEGAL_ASSESSMENT.md`](DOCUMENT_TRANSLATION_LEGAL_ASSESSMENT.md) | Internal legal pre-assessment in German: every statement tagged FACT / INTERPRETATION / OPEN / EXTERNAL, sources with dates, roles per stage, Art. 6/9, consent, DPIA necessity, A1a, Art. 28 change list, B1, B7, third parties, retention, international transfer, MDR, AI Act, risk matrix. Not legal advice and not an approval |
